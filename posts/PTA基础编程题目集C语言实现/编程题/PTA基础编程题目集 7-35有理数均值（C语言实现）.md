...
summary: 本文是 PTA 编程题"7-35 有理数均值"的题解，涵盖题目描述、输入输出格式及纯 C 语言实现，展示逐个读入分数字符串、按 '/' 是否存在把有理数解析为 (分子,分母) 长整数对，通过通分累加并逐步约分；最终将总和分母 ×N 求平均值，再用辗转相除法 gcd 约成最简，保证分母为正、分母为 1 仅输出分子。
weight: 135
---
