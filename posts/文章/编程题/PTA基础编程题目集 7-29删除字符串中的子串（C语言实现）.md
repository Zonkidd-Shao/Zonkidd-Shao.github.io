...
tags: PTA, C语言, 编程题, 算法
category: 算法题解
summary: 本文是 PTA 编程题"7-29 删除字符串中的子串"的题解，涵盖题目描述、输入输出格式及 C 语言实现，展示用 `fgets` 读取含空格的两行字符串 S1/S2，通过外层循环反复查找+内层前移覆盖的方式，重复删除 S1 中所有出现的子串 S2，直到 S1 中再无 S2 为止的朴素删除算法。
weight: 129
---
