...
date: 2026-08-07
description: "本文是 PTA 编程题'7-31 字符串循环左移'的题解，涵盖题目描述、输入输出格式及 C 语言实现，展示使用 `fgets` 读取含空格字符串并去除换行、对左移位数 `N %= len` 处理 N≥长度的情况，最终通过'先打印 [n, len) 再打印 [0, n)'两段直接拼接输出循环左移结果的巧妙方法。"
categories:
...
tags:
  ...
  - 算法
weight: 131
slug: PTA基础编程题目集-7-31字符串循环左移（C语言实现）
---
