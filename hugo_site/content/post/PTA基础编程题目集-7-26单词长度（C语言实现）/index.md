...
date: 2026-08-07
description: "本文是 PTA 编程题'7-26 单词长度'的题解，涵盖题目描述、输入输出格式及 C 语言实现，展示使用 `scanf('%c')` 逐字符读取直到遇到 `.` 为止，并通过长度计数器 `len` 和首项标志 `first` 实现：按空格切分单词、跳过连续空格、输出每个单词长度且行末无多余空格的方法。"
categories:
...
tags:
  ...
  - 算法
weight: 126
slug: PTA基础编程题目集-7-26单词长度（C语言实现）
---
